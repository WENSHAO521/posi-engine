...
test('isCitable follows the PJR-SPEC.md § 5 document-type table', () => {
  assert.equal(isCitable({ document_type: 'research-article', retracted: false }), true)
  assert.equal(isCitable({ document_type: 'editorial', retracted: false }), false)
  assert.equal(isCitable({ document_type: 'correction', retracted: false }), false)
})

test('a retracted work is never citable, even if its document_type normally would be', () => {
  assert.equal(isCitable({ document_type: 'research-article', retracted: true }), false)
})
...
test('calculatePci matches the worked example in PJR-SPEC.md § 6', () => {
  ...
  const works = [
    ...Array.from({ length: 150 }, () => ({ document_type: 'research-article', retracted: false, citations_in_year: 0 })),
    ...Array.from({ length: 170 }, () => ({ document_type: 'research-article', retracted: false, citations_in_year: 0 })),
  ]
...
test('calculatePci returns a null ratio (not divide-by-zero) with no citable items', () => {
  const result = calculatePci([{ document_type: 'editorial', retracted: false, citations_in_year: 5 }])
